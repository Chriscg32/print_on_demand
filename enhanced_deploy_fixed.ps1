...
function Validate-Installation {
    ...
    try {
        ...
        if ($service.Status -eq "Running") {
            $validationResults += "[SUCCESS] Service '$ServiceName' is running"
        } else {
            $validationResults += "[FAILED] Service '$ServiceName' is not running (Status: $($service.Status))"
        }
    } catch {
        $validationResults += "[FAILED] Service '$ServiceName' not found"
    }
    
    # Check if database exists
    if (Test-Path $DatabasePath) {
        $validationResults += "[SUCCESS] Database exists at $DatabasePath"
    } else {
        $validationResults += "[FAILED] Database not found at $DatabasePath"
    }
    
    # Check firewall rule
    $firewallRule = Get-NetFirewallRule -DisplayName "POD-API-Inbound" -ErrorAction SilentlyContinue
    if ($firewallRule) {
        $validationResults += "[SUCCESS] Firewall rule 'POD-API-Inbound' exists"
    } else {
        $validationResults += "[FAILED] Firewall rule 'POD-API-Inbound' not found"
    }
